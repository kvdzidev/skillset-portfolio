...
void printHorizontal(){
        ...
        printHorizonthelp(this->root);
    }
    void printHorizonthelp(TreeNode* node, int depth = 0, char slash ='='){
        if(node == nullptr)return;
            printHorizonthelp(node->right, depth + 1, '/');
        for(int i=0; i<depth;i++) std::cout << ' ';
        std::cout << slash << node->val << std::endl;
        printHorizonthelp(node->left,depth+1,'\\');
    }
// ROZWIĄZANIE KONIEC
